from flask import Flask
from flask_sqlalchemy import SQLAlchemy
import flask
from flask_cors import cross_origin #跨域
import json
app = Flask(__name__)
app.config["SQLALCHEMY_DATABASE_URI"] = 'oracle://wh:Admin135@127.0.0.1:1521/orcl'
app.config['SQLALCHEMY_ECHO'] = True
app.config["SQLALCHEMY_TRACK_MODIFICATIONS"] = False
app.config["JSON_AS_ASCII"] = False
db = SQLAlchemy(app)

# 分类
class CaiLiao(db.Model):
    __tablename__ = "第十八批_医保医用耗材汇总"
    货物码 = db.Column(db.String(50),primary_key=True)
    原27位码 = db.Column(db.String(50))
    现27位码   = db.Column(db.String(50))
    单件产品名称 = db.Column(db.String(50))
    注册备案号 = db.Column(db.String(50))
    规格 = db.Column(db.String(50))
    型号 = db.Column(db.String(50))
    耗材企业 = db.Column(db.String(60))
    原码状态 = db.Column(db.String(50))
    类型 = db.Column(db.String(50))


def finds(CLCode):
    result = CaiLiao.query.filter(CaiLiao.原27位码 == CLCode).all()
    NLst = []
    for x in range(len(result)):
      CLInfo = {'单件产品名称': result[x].单件产品名称,
              '原27位码': result[x].原27位码,
              '原码状态': result[x].原码状态,
              '27位码': result[x].现27位码,
              '类型': result[x].类型,
              '注册备案号': result[x].注册备案号,
              '规格': result[x].规格,
              '型号': result[x].型号,
              '耗材企业': result[x].耗材企业
              }
      NLst.append(CLInfo)
    strjs = json.dumps(NLst, indent=2, ensure_ascii=False)
    return strjs



@app.route('/find')
def hello():
    return  flask.render_template('demo.html')


@app.route('/lookup', methods=['POST'])
@cross_origin()
def ChaZhao():
    data = flask.request.get_data()
    datalist = json.loads(data)
    CLInfo = datalist['data'][0]['CLCode']
    Fresults=finds(CLInfo)
    return Fresults








if __name__ == '__main__':

    app.run(host='0.0.0.0',port=8800)